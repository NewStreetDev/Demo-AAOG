...
import type { AgroTask } from '../../types/agro.types';

interface AgroTaskListProps {
  tasks: AgroTask[];
  maxItems?: number;
  onViewAll?: () => void;
}

function getPriorityColor(priority: AgroTask['priority']): string {
  const colors = {
    high: 'bg-red-500',
    medium: 'bg-amber-500',
    low: 'bg-green-500',
...
function getTaskTypeIcon(type: AgroTask['type']): string {
  const icons = {
    planting: '🌱',
    irrigation: '💧',
    fertilization: '🧪',
...
export default function AgroTaskList({ tasks, maxItems = 5, onViewAll }: AgroTaskListProps) {
